...
with a 1,000,000-token request ceiling, six active sequences, 0.85 GPU memory
utilization, NVFP4 DS-MLA KV cache, and the checkpoint's own five-token dSpark
draft. A plain `docker compose up -d` or `./launch.sh --nodes 1 --model k2`
keeps that default.
...
Follow startup with `docker logs -f ds4-mia-k2-tp1`, or use Compose for the
default K2-v1 service:
...
| --- | ---: | --- |
| `MODEL_KIND` | `k2` | K2 calibrated v1 target and dSpark draft |
