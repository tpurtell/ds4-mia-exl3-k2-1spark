...
def stream_one(base_url, model, prompt, temperature, output_tokens):
    instruction = (
...
async def run_case(
    ...
    results = await asyncio.gather(*[
        asyncio.to_thread(
            stream_one, base_url, model, p, temperature, output_tokens
        )
        for p in prompts
    ])
...
async def main():
    ...
    for rep in range(args.repeat):
        ...
        print(f"trial {rep}: c={case['concurrency']} p={args.prompt} "
              ...
              f"pt={[r['prompt_tokens'] for r in case['requests']]} "
